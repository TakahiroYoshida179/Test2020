...
int main(int argc, char *argv[]) {
	...
	//宣言
	int result;
        uint8_t *fqdn="tun1.dc.ntm.jp";

	if (argc < 3) {
		fprintf(stderr, "Usage : %s <s or c> <nh or h> <Dst IPaddress>\n", argv[0]);
		fprintf(stderr, "ex:\n %s s h\n %s c nh\n", argv[0], argv[0]);
		exit(1);
	}
...
forwardloop_svr_udp(int sock)
{
        ...
        for (;;) {
                ...
                rlen = recvfrom(sock, recv_buf, BUFF_SIZE, 0, 
                ...
                slen = sendto(sock, recv_buf, rlen, 0,
                        (struct sockaddr *) &peer_addr,
                                            peer_addr_len);
                if (slen != rlen) {
                        perror("sendto()");
 ...
 * "recvfrom -> sendto" のループをする.HeatRun Ver.
 * 絵が出ます
 * 返り値:
 *   0 -> 正常終了
 *  -1 -> 何かしらのエラー
...
socket_clt_udp(const uint8_t *fqdn, struct addrinfo **res0)
{
	...
	printf("Go socket_clt_udp\n");
        ...
        result = getaddrinfo(fqdn, PORT_NUM, &hints, &(*res0));
        if (result == -1) {
                fprintf(stderr, "getaddrinfo(): %s\n", gai_strerror(result));
                return -1;
        }
	...
	printf("client sock()\n");
	sock = socket((*res0)->ai_family, (*res0)->ai_socktype, 0);
        if (sock == -1) {
                perror("socket()");
                freeaddrinfo(*res0);
                close(sock);
                    ...
                    struct addrinfo **res)
{
        ...
        if (ipver == 0) {
                ((struct sockaddr_in *)(*res)->ai_addr)->sin_port = htons(23456);
        } else {
                ((struct sockaddr_in6 *)(*res)->ai_addr)->sin6_port = htons(23456);
        }
        ...
        for (;;) {
                ...
                slen = sendto(sock, send_buf, strlen(send_buf), 0,
...
/*
 * forwardloop_clt_udp
 * "sendto -> recvfrom" のループをする.HeatRun Ver.i
 * 絵が出ます
 * 返り値:
 *   0 -> 正常終了
 *  -1 -> 何かしらのエラー
